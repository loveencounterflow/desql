(function() {
  'use strict';
  var CND, FS, GUY, H, PATH, SQL, add_views, badge, debug, echo, help, info, rpr, urge, warn, whisper;
  ...
  this.DBay_trash = (clasz = Object) => {
    return class extends clasz {
      ...
      //---------------------------------------------------------------------------------------------------------
      trash_to_sql(cfg) {
        var iterator, overwrite, path, row;
        this.types.validate.dbay_trash_to_sql_cfg((cfg = {...this.constructor.C.defaults.dbay_trash_to_sql_cfg, ...cfg}));
        this.create_trashlib();
        this.db.setv('_use_dot_cmds', cfg._use_dot_cmds);
        ({path, overwrite} = cfg);
        iterator = this.db.query(this.db._trash_select_from_statements);
        if ((cfg.path == null) || (cfg.path === false)) {
          if (cfg.walk) {
            return iterator;
          }
          return ((function() {
        ...
        return this._trash_with_fs_open_do(path, 'sql', overwrite, ({path, fd}) => {
          for (row of iterator) {
      ...
      trash_to_sqlite(cfg) {
        ...
        var buffer, overwrite, path, sql, sqlt;
        this.types.validate.dbay_trash_to_sqlite_cfg((cfg = {...this.constructor.C.defaults.dbay_trash_to_sqlite_cfg, ...cfg}));
        ({path, overwrite} = cfg);
        sql = this.trash_to_sql({
          walk: false,
          path: false,
          _use_dot_cmds: false
        });
        sqlt = this.db.constructor.new_bsqlt3_connection();
        sqlt.exec(sql);
        buffer = sqlt.serialize();
        if ((cfg.path == null) || (cfg.path === false)) {
          return buffer;
        }
        return this._trash_with_fs_open_do(path, 'sqlite', overwrite, ({path, fd}) => {
      ...
      _trash_with_fs_open_do(path, extension, overwrite, fn) {
        var R, fd;
        path = this._trash_get_path(path, extension);
        fd = FS.openSync(path, overwrite ? 'a' : 'ax');
        try {
          (R = fn({path, fd}));
        } finally {
          FS.closeSync(fd);
      ...
      _trash_get_path(path, extension) {
        var type;
        if ((type = this.types.type_of(path)) === 'text') {
          return path;
        }
        if (path !== true) {
          throw new this.db.E.DBay_internal_error('^desql/trash@1^', `expected a text or \`true\`, got a ${type}`);
        }
        clasz = this.db.constructor;
        return PATH.join(clasz.C.autolocation, this.db.rnd.get_random_filename(extension));
      }

      //---------------------------------------------------------------------------------------------------------
      create_trashlib() {
        if (this._trash_created) {
          return null;
        }
        add_views(this.db);
        this._trash_created = true;
        return null;
      }
...
create view dbay_unique_fields as select
  ...
  ;`);
    //---------------------------------------------------------------------------------------------------------
    db(SQL`drop view if exists _dbay_fields_1;
create view _dbay_fields_1 as select
  ...
  order by table_nr, field_nr;`);
    //---------------------------------------------------------------------------------------------------------
    db(SQL`drop view if exists _dbay_fields_2;
...
create view dbay_fields as select
  ...
  order by table_nr, field_nr;`);
    //---------------------------------------------------------------------------------------------------------
    db(SQL`drop view if exists _dbay_foreign_key_clauses_1;
create view _dbay_foreign_key_clauses_1 as select
  ...
  order by from_table_name, fk_id, fk_idx;`);
    //---------------------------------------------------------------------------------------------------------
    db(SQL`drop view if exists _dbay_foreign_key_clauses_2;
create view _dbay_foreign_key_clauses_2 as select distinct
  ...
  order by from_table_name, fk_id, fk_idx;`);
    //---------------------------------------------------------------------------------------------------------
    db(SQL`drop view if exists _dbay_foreign_key_clauses_3;
...
create view dbay_foreign_key_clauses as select
  ...
  order by from_table_name, fk_nr;`);
    //---------------------------------------------------------------------------------------------------------
    db(SQL`drop view if exists _dbay_primary_key_clauses_1;
...
create view dbay_primary_key_clauses as select distinct
  ...
  order by p1.table_nr;`);
    //---------------------------------------------------------------------------------------------------------
    db(SQL`drop view if exists _dbay_field_clauses_1;
...
create view dbay_drop_table_clauses as select
  ...
  from dbay_tables;`);
    //---------------------------------------------------------------------------------------------------------
    db(SQL`drop view if exists _dbay_create_table_statements_1;
create view _dbay_create_table_statements_1 as
  ...
  order by section_nr, table_nr, part_nr, lnr;`);
    //---------------------------------------------------------------------------------------------------------
    db(SQL`drop view if exists _dbay_create_table_statements_2;
create view _dbay_create_table_statements_2 as select
  ...
  order by section_nr, table_nr, part_nr, r1.lnr;`);
    //---------------------------------------------------------------------------------------------------------
    db(SQL`drop view if exists _dbay_create_table_statements_3;
create view _dbay_create_table_statements_3 as select
  ...
  order by lnr, tail;`);
    //---------------------------------------------------------------------------------------------------------
    db(SQL`drop view if exists _dbay_create_table_statements_4;
create view _dbay_create_table_statements_4 as select
  ...
  order by lnr, tail;`);
    (() => {      //---------------------------------------------------------------------------------------------------------
      var skip;
      skip = false;
      return db.create_table_function({
