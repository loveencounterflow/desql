(function() {
  'use strict';
  var CND, GUY, SQL, X, _show_antler_tree, antler_types, badge, debug, echo, equals, help, info, isa, position_from_node, queries, rpr, show_antler_tree, show_overview, tabulate, to_snake_case, type_of, type_of_antler_node, types, urge, validate, validate_list_of, warn, whisper, xrpr;
...
select 'helo world' as greetings;`,
  ...
  _show_antler_tree = function(query, tree, parent, level, R) {
    var dent, i, id, len, node, position, ref, type, type_entry, type_entry_type;
    dent = '  '.repeat(level);
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      ...
      debug('^5600-1^', position, rpr(query.slice(position.start.idx, +position.stop.idx + 1 || 9e9)));
