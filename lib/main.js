...
union all select
  ...
  returning *;`)
      ...
      parse(query) {
        ...
        // stack           = [ @constructor.typedata.by_full_name.query.t2, ]
      ...
      _build_tree(qid, query, antlr, parent, tree) {
        var branch, flat_node, i, len, node, path, position, ref, ref1, tdbfn, txt, type, upid;
        tdbfn = this.constructor.typedata.by_full_name;
        ref = antlr.children;
        for (i = 0, len = ref.length; i < len; i++) {
          ...
          // path            = ( parent.path ? '' ) + ',' + tdbfn[ type ]?.t2 ? type
          path = 'XXX';
