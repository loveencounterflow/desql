(function() {
  ...
  ( 'window_frame',                       'wf'        ),
  ( 'window_ref',                         'wr'        );`);
        ref = ['k select', 'k where', 'k insert', 'k insert into', 'k generated', 'k generated always', 'k order', 'k order by', 'k order by ascending', 'k order by descending', 'k join', 'k join using', 'k join on', 'k window', 'k window as', 'k with', 'i select from tbl name', 'i select from tbl alias', 'i select from col name', 'i select from col alias', 'i order by tbl name', 'i order by col name', 'i join on tbl name', 'i join on tbl alias', 'i join on col name'];
        for (i = 0, len = ref.length; i < len; i++) {
          name = ref[i];
          code = this._code_from_tcat_name(name);
          try {
